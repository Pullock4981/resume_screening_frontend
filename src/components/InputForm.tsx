...
import { Play, FileText, Table, Plus, X, AlertCircle, Sparkles, Briefcase, Copy, Check, Lock } from 'lucide-react';
    ...
    <div className={`w-full border backdrop-blur-xl rounded-2xl p-6 transition-colors duration-200 ${
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {/* Field 2: Master Database Sheet URL (Fixed & Read Only) */}
          <div>
            <label className={`block text-xs font-semibold mb-1.5 flex items-center justify-between ${
              ...
              <span className="text-[10px] font-mono text-amber-500 bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded flex items-center gap-1">
            ...
            <div className="relative flex items-center">
              <input
                ...
                className={`w-full border rounded-xl pl-3 pr-20 py-2 text-xs font-mono transition select-all ${
                  isDark
                    ? 'bg-slate-950/90 border-slate-800 text-slate-300 cursor-not-allowed'
              ...
              <button
                ...
                className={`absolute right-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold flex items-center gap-1 transition ${
                  copiedMasterUrl
                    ? 'bg-emerald-500 text-white'
                    : isDark
                    ? 'bg-indigo-600/30 hover:bg-indigo-600/50 text-indigo-400 border border-indigo-500/30'
